...
def Economic():
    ...
    # df = pd.read_excel(file_path, dtype=str).fillna("")
    #df = df[['ACCOUNT', 'Len', 'EFFDT', 'EFFDT_Year', 'EFF_STATUS', 'DESCRLONG_KHM', 'នៅក្រោមខេត្ត']]
    ...
    #     mask2 = filtered_df["ACCOUNT"].str.match(pattern, case=False, na=False)
    #     mask3 = filtered_df["DESCRSHORT_ENG"].str.contains(text_search, case=False, na=False)
    #     filtered_df = filtered_df[mask1 | mask2 | mask3]
    if text_search:
        pattern = f"^{text_search}"

        if filtered_df["ACCOUNT"].str.match(pattern, case=False, na=False).any():
            filtered_df = filtered_df[filtered_df["ACCOUNT"].str.match(pattern, case=False, na=False)]
    ...
    # if text_search:
    #     mask1 = filtered_df["DESCRLONG_KHM"].str.contains(text_search, case=False, na=False)
    #     mask2 = filtered_df["ACCOUNT"].str.contains(text_search, case=False, na=False)
        ...
        for i, col_df in enumerate([col1_df, col2_df, col3_df]):
            with cols[i]:
                for _, row in col_df.iterrows():
                    province = str(row['នៅក្រោមជំពូក']).strip() if pd.notna(row['នៅក្រោមជំពូក']) else None
                    province_text = (
                        f"<p style='font-size: 16px;'><b style='color:#28a745;'>🏛 ស្ថិតនៅក្រោមជំពូក:</b> {province}</p>"
                        if province else ""
                    )
                    ...
                    <div style="border: 1px solid #ddd; border-radius: 10px; padding: 20px; margin-bottom: 20px; background-color: #f9f9f9; height: 280px; overflow-y: auto;">
                        <h4 style="color: #1f2937; font-size: 22px; height: 50px; overflow: hidden; text-overflow: ellipsis;">{row['ACCOUNT']}</h4>
