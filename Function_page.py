...
def Functionpage():
    ...
    # # Apply year + status filtering after inputs are defined
    # if option != "All":
    ...
    if text_search:
        ...
        if filtered_df["CLASS_FLD"].str.match(pattern, case=False, na=False).any():
            filtered_df = filtered_df[filtered_df["CLASS_FLD"].str.match(pattern, case=False, na=False)]
    ...
    #     mask2 = filtered_df["CLASS_FLD"].str.contains(text_search, case=False, na=False)
        ...
        for i, col_df in enumerate([col1_df, col2_df, col3_df]):
            with cols[i]:
                for _, row in col_df.iterrows():
                    province = str(row['នៅក្រោមមុខងារ']).strip() if pd.notna(row['នៅក្រោមមុខងារ']) else None
                    province_text = (
                        f"<p style='font-size: 16px;'><b style='color:#28a745;'>🏛 ស្ថិតនៅក្រោមមុខងារ:</b> {province}</p>"
                        if province else ""
                    )
                    ...
                    <div style="border: 1px solid #ddd; border-radius: 10px; padding: 20px; margin-bottom: 20px; background-color: #f9f9f9; height: 280px; overflow-y: auto;">
                        <h4 style="color: #1f2937; font-size: 22px; height: 50px; overflow: hidden; text-overflow: ellipsis;">{row['CLASS_FLD']}</h4>
